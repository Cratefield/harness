//! Shared helpers for the Postgres integration tests: a per-test throwaway
//! database on the server named by `FZ_TEST_POSTGRES_URL`, so tests never
//! see each other's tables. Tests skip with a printed reason when the
...
pub(crate) fn skip_reason() -> String {
...
/// A fresh database named `fz_<tag>_<n>`; call [`TempDb::finish`] to drop
/// it.
...
impl TempDb {
    pub(crate) async fn create(tag: &str) -> Option<Self> {
        let base = base_url()?;
        let admin = sqlx::PgPool::connect(&base)
            .await
            .expect("FZ_TEST_POSTGRES_URL server accepts connections");
        // Unique per process and per call: a previous run's leaked
        // databases (a test that panicked before `finish`) never collide.
        let database = format!(
            "fz_{tag}_{}_{}",
            std::process::id(),
    ...
    pub(crate) async fn assert_postgres_16(&self) {
    ...
    pub(crate) async fn finish(self) {
        ...
        if let Ok(dropper) = sqlx::PgPool::connect(&base_url().expect("set in create")).await {
            dropper
                .execute(
