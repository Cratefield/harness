-- auth issue #9: token issuing, Postgres form. Two schema changes, both
-- additive in effect:
--
...
--    session. The rebuild-the-table-and-copy pattern matches the
--    sqlite migration step for step, so both dialects end on the same
--    shape; Postgres differs only in the byte column type: BYTEA, not
--    BLOB (harness issue #18, ADR 0004). Harness portable SQL subset
--    (harness issue #8): plain DDL, no dialect functions.
