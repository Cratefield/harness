-- auth issue #5: the schema, Postgres form. Identical to the sqlite set
-- in one-word difference: BYTEA, not BLOB, which Postgres does not have
-- (harness issue #18, ADR 0004). Column set, constraints, comments and
-- the four indexes match migrations/sqlite/0001_init.sql; the tracking
-- id is the same "0001" so `<module>/<id>` stays stable across dialects.
...
CREATE TABLE IF NOT EXISTS credentials (           -- passkeys and password hashes
    ...
    passkey_public_key_cose BYTEA,                 -- COSE key: public by design
    passkey_sign_count INTEGER,
    passkey_aaguid BYTEA,
...
CREATE TABLE IF NOT EXISTS sessions (
    id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL REFERENCES users(id),
    token_hash BYTEA NOT NULL UNIQUE,              -- sha256 of the cookie value; the value itself is never stored
    created_at TEXT NOT NULL,
