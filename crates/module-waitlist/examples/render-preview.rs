//! Writes every default waitlist template with sample data to
...
fn main() {
    ...
    let confirm = json!(ConfirmMailData {
        ...
        confirm_url: "https://api.factory0.ventures/v1/waitlist/confirm?token=sample".to_owned(),
        brand: brand.clone(),
    });
    let confirmed = json!(ConfirmedMailData {
        venture: "factory0".to_owned(),
        product: "kontinuum".to_owned(),
        email: "nick@example.com".to_owned(),
        position: 42,
        status_url: "https://api.factory0.ventures/v1/waitlist/status?token=sample".to_owned(),
        brand,
    });

    for (id, data) in [
        ("waitlist/confirm", &confirm),
        ("waitlist/confirmed", &confirmed),
    ] {
