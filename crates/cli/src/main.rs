...
//! `fz build <manifest>`, which *generates* a venture from a manifest;
//! `fz client-ts` (issue #155), which generates the typed TypeScript
//! client from the contract a venture serves; the manifest workflow —
//! `fz plan` / `deploy` / `add` / `init` / `verify` (harness #140) —
//! which works on the manifest and its on-disk records; and `fz push`
//! (issue #184), which reads the venture's environment.
