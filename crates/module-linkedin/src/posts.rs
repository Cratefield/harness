...
use cratefield_core::{Clock, Database, ModuleContext, Problem, Scope};
...
pub(crate) async fn create(
    ...
    cratefield_core::Json(raw): cratefield_core::Json<Value>,
...
pub(crate) async fn edit(
    ...
    headers: HeaderMap,
    Path(id): Path<String>,
    cratefield_core::Json(body): cratefield_core::Json<Value>,
) -> Result<Response, Problem> {
    handlers::admin(&state, &headers)?;
    let ctx = state.ctx.as_ref();
    let settings = state.settings();
