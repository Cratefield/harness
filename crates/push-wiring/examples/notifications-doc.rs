//! Generates `docs/NOTIFICATIONS.md` from `cratefield-push-wiring`'s
        ...
        if current != generated {
            eprintln!(
                "docs/NOTIFICATIONS.md is stale; regenerate with `cargo run -p \
                 cratefield-push-wiring --example notifications-doc` and commit it"
            );
            std::process::exit(1);
        }
        println!(
            "docs/NOTIFICATIONS.md is up to date ({} variables)",
            PUSH_ENV.len()
        );
        return;
    }

    std::fs::write(&path, generated).expect("write docs/NOTIFICATIONS.md");
