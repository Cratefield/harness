...
use factory0_core::{
    Clock, Database, Decision, Kid, MapConfig, Payload, Ports, Scope, Signer, Statement,
...
fn kit() -> TestHarness {
    TestHarness::new(vec![Box::new(EmailSignup::new())])
}
...
fn admin_kit() -> TestHarness {
    TestHarness::with_ports(vec![Box::new(EmailSignup::new())], |ports| {
...
async fn captcha_denial_is_400_captcha_failed() {
    let kit = TestHarness::with_ports(vec![Box::new(EmailSignup::new())], |ports| {
...
async fn double_opt_in_off_confirms_immediately_without_mail() {
    let kit = TestHarness::new(vec![Box::new(EmailSignup::new().double_opt_in(false))]);
    let response = signup(&kit, "nick@example.com").await;
    assert_eq!(response.status, StatusCode::ACCEPTED);
...
async fn welcome_mail_is_sent_on_confirm_when_enabled() {
    let kit = TestHarness::new(vec![Box::new(EmailSignup::new().welcome_on_confirm(true))]);
    signup(&kit, "nick@example.com").await;
    assert_eq!(kit.mailer.sent().len(), 1);
    let confirm_path = path_of(&links(&kit)[0]);
    request(&kit.router, Method::GET, &confirm_path, None).await;
