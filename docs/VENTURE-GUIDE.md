# Venture guide — from `fz init` to production
...
path from `fz init` to a production API on `api.<your-domain>`: the
manifest describes the venture, `fz build` generates the crate from it,
and every step after that is ordinary Cargo and wrangler.
...
conditions here; they are marked **human**. The rest are executed by
[`tools/doc-commands.sh`](../tools/doc-commands.sh), including the
`fz init` / `fz add` opening of step 1, which runs in a fresh temporary
directory. Substitute your venture's name for `venture-example` and your
...
- a GitHub account with access to the repository your venture lives in
  (deploys are GitHub Actions workflows).

## 1. Create the venture

A venture starts as a manifest. `fz init` writes one — name and host, no
modules yet — and `fz add` appends a module to it. Both touch the
manifest and nothing else. This is run for real in CI, in a fresh
directory, by [`tools/doc-commands.sh`](../tools/doc-commands.sh):

```
fz init --name venture-example --host venture-example.factory0.dev
fz add waitlist
```
...
The manifest is the one file a venture edits:

```json
{
  "name": "venture-example",
  "host": "venture-example.factory0.dev",
  "cors_origins": ["https://venture-example.factory0.dev"],
  "modules": [
    "waitlist",
    { "slug": "email-signup" }
  ]
...
Then generate the crate:

```
# --out defaults to dist; --harness-path ../.. points the generated
# Cargo.toml at this workspace instead of the published cratefield
fz build venture.json --out ventures/<venture> --harness-path ../..
```

`fz build` writes the crate and nothing else:

```
venture.json     the manifest — the only file a venture edits          ← edit this
Cargo.toml       GENERATED — cratefield, worker, and the venture's own fz bin
src/lib.rs       GENERATED — the harness() composition and the fetch/scheduled events
src/fz_main.rs   GENERATED — the venture-linked fz: migrations collect, doctor
wrangler.toml    GENERATED — worker-build --release, the DB binding, migrations/
provenance.json  GENERATED — module pins, composition hash, who built what when
src/tables.rs    GENERATED — only when the manifest declares tables
seed.sql         GENERATED — only when the manifest sets seed_sql
```

Every generated source file opens with a `GENERATED by fz build` header,
and the deal is the one the header states: when the manifest changes,
re-run `fz build` instead of editing them. The build never writes
`migrations/` — those come from `fz migrations collect` (step 4). Two
honest footnotes: `wrangler.toml` is regenerated too, so the
account-specific lines added to it in step 3 are re-applied after a
rebuild; and `--harness-path` is an in-this-repository convenience —
leave it off and the generated crate depends on the published
`cratefield` from crates.io.

**Human.** Add the venture to the workspace `members` in the root
`Cargo.toml` — `ventures/*` is deliberately not globbed, so the new
directory is invisible to `cargo` until you list it beside
`ventures/cratefield-waitlist` (ADR [0013](adr/0013-one-repository.md)).

`examples/tables-canary` is the pipeline above, run for real and
committed: a generated venture, regenerated with
`cargo run -p cratefield-manifest --example write-venture -- examples/tables-canary/venture.json examples/tables-canary`
and byte-compared against the generator by
`crates/manifest/tests/canary_is_current.rs`. CI compiles it as a
workspace member, and `examples/tables-canary/tests/boots.rs` composes
its generated `harness()` — so the generator cannot drift from what a
venture looks like without a red build.

(`ventures/cratefield-waitlist` is older than the generator: hand-written,
composing in `src/lib.rs`. It stays in the workspace and builds, but new
ventures follow the manifest path above.)

**What catches a bad composition.** `Harness::build()` — the call behind
`fz migrations collect`, `fz doctor`, and the first request the Worker
serves — refuses a module requiring a port the runtime does not provide,
two modules claiming one table, duplicate module names, unknown or
duplicated port declarations, a `harness_api` mismatch, and a production
venture that is not production-ready. A generated composition ends in
`.expect("generated venture harness is valid")`, so a bad manifest is a
panic at the first compose — and `examples/tables-canary/tests/boots.rs`
performs that compose under `cargo test --workspace`, which turns it
into a red CI build instead of a production surprise.

`fz doctor` does not re-check any of that: by the time it runs,
`Harness::build()` has already succeeded by construction. Its checks are
the deployment-level rules on a harness that did compose —
migration-lockfile consistency, the production captcha rule, push
wiring, sidecar mounts, and the portable-SQL and card-data lints.

## 2. Run it locally

From the venture directory (the runs below are against
`examples/venture`, the hand-written smallest venture):

```
$ cargo test          # compiles the venture and runs its tests
```
...
environment; this guide names them `<venture>-api-staging` and
`<venture>-api`:
...
From the venture directory — `fz build` gives every generated venture an
`fz` bin target (`src/fz_main.rs`), so this runs where the manifest is
(see the [cratefield-cli README](../crates/cli/README.md)):
...
migration to its global file, so adding a module later appends. From the
repository root rather than the venture directory, name the package —
several crates define a bin called `fz`, so a bare `--bin fz` is
ambiguous there: `cargo run -p <venture> --bin fz -- doctor`. Then
...
The four required secrets, plus two optional ones that shape the signer's
key ring (issue #137, ADR 0014; architecture section 9):
...
a routes/custom-domain entry in `wrangler.toml` or the dashboard.
`public_url` and `cors_origins` come from the manifest, so change them
...
and your site writes no form code. `examples/venture` mounts it in
`src/lib.rs` (`.ui(Ui::from_spec(include_str!("../ui.json")))`); the
generator does not emit a UI mount today, so that line is a hand edit —
one a rebuild overwrites, like any change to a generated file. Two
ways in:
...
`fz build` prints the rest of the path and stops there — it constructs,
never deploys (issue #142): `worker-build --release` compiles the
venture to wasm, then `wrangler deploy` stands up the Worker. Migrations
are applied as in step 4, not by `deploy` — the auth workflow below does
it in a pre-deploy step. Both commands need what this guide cannot run
for you — the toolchain's wasm target, and Cloudflare credentials.
...
|---|---|---|
| `fz init` → `fz add` → `fz build`, add to workspace `members` | no | you |
| `cargo test`, `fz doctor` | no | CI's test job runs `cargo test --workspace` on every PR, composition test included; `fz doctor` is your pre-deploy gate |
...
| Production secrets (`wrangler secret put`) | **yes** | human with wrangler auth |
| New module versions on crates.io | **yes** | harness maintainers merge the release-plz PR; Renovate opens venture bumps |
| Deploys | **yes** | no venture deploy workflow ships: `worker-build --release` + `wrangler deploy`, or copy `.github/workflows/deploy-auth.yml` |
