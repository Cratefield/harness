...
> **The template is currently a README and a banner image** — issue #408.
> It was a separate GitHub template repository and only its documentation
> came across when #166 folded the repositories together, so copying it
> gives you those two files and nothing else. The shape below is what it
> is meant to contain, not what you get.
>
> Two things that do exist and build today, until that is settled:
> `ventures/cratefield-waitlist` is a real venture in this repository
> (note that it composes the harness in `src/lib.rs` rather than in a
> separate `src/harness.rs`), and `examples/tables-canary` is a complete
> venture generated by `fz build`, committed, and built and booted by
> CI.
...
fails `Harness::build()`. That composition check is real; the test named
above is not — `harness_builds` is in no source file in this repository,
so nothing currently turns a bad composition into a `cargo test` failure
for a venture (issue #408). Until it exists, `fz doctor` and the
venture's own CI are what catch it.
